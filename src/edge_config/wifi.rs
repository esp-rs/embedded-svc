...
pub fn register<R: Registry>(registry: R, pref: &str, default_role: Option<auth::Role>) -> Result<R> {
    ...
    registry
        .at(prefix("")).get(get_status)?
        .at(prefix("/scan")).post(scan)?
        .at(prefix("/caps")).get(get_capabilities)?
        .at(prefix("/conf")).get(get_configuration)?
        .at(prefix("/conf")).put(set_configuration)?
        .at(pref).middleware(with_permissions(default_role))
}
