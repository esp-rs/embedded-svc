...
use crate::{http::cookies::Cookies, mutex::*};
...
impl fmt::Display for SessionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SessionError::MaxSessionsReachedError => write!(f, "Max number of sessions reached"),
        }
    }
}
...
where
    ...
    S: Default,
{
    fn get_existing_id<'a>(&self, req: &'a impl Request) -> Option<&'a str> {
        req.header("Cookie")
