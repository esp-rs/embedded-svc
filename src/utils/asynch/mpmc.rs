...
struct ChannelState<T, const N: usize> {
    queue: Deque<T, N>,
    receiver_waker: SingleWakerRegistration,
    senders_waker: SingleWakerRegistration,
}
...
impl<T, const N: usize> ChannelState<T, N> {
    const fn new() -> Self {
        ChannelState {
            queue: Deque::new(),
            receiver_waker: SingleWakerRegistration::new(),
            senders_waker: SingleWakerRegistration::new(),
        }
    }
...
where
    M: RawMutex,
{
    inner: Mutex<M, ChannelState<T, N>>,
}
